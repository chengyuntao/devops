...
import com.quxie.store.controller.ex.*;
import com.quxie.store.entity.User;
import com.quxie.store.entity.model.LoginBody;
import com.quxie.store.service.IUserService;
...
public class UserController extends BaseController {
    ...
    @PostMapping("/reg")
    public JsonResult<Void> reg(@RequestBody LoginBody loginBody) {
        // 调用业务对象执行注册
        User user = new User();
    ...
    */
    ...
    @PostMapping("/login")
    public JsonResult<User>login(@RequestBody LoginBody loginBody, HttpSession session) {
        // 调用业务对象的方法执行登录，并获取返回值
        User data = userService.login(loginBody.getUsername(), loginBody.getPassword());
        //登录成功后，将uid和username存入到HttpSession中
        session.setAttribute("uid", data.getUid());
        session.setAttribute("username", data.getUsername());
        // System.out.println("Session中的uid=" + getUidFromSession(session));
